...
class KdV(PDE):
    ...
    def __init__(
        ...
        self.tmax = 20.0 if tmax is None else tmax
        ...
        self.L = 128.0 if L is None else L
    ...
    def __repr__(self):
        return f"1D_KdV_delta={self.delta}"

    def pseudospectral_reconstruction(
        self, t: float, u: np.ndarray, L: float
    ) -> np.ndarray:
        """
        ...
        # Compute du/dt.
        dudt = -u * ux - (self.delta * self.delta * uxxx)
        return dudt
...
class KS(PDE):
    ...
    def __init__(
        ...
        # Start and end time of the trajectory
        self.tmin = 0 if tmin is None else tmin
        self.tmax = 40.0 if tmax is None else tmax
        # Sine frequencies for initial conditions
        self.lmin = 1 if lmin is None else lmin
        self.lmax = 3 if lmax is None else lmax
        # Number of different waves
        self.N = 10
        # Length of the spatial domain
        self.L = 64.0 if L is None else L
        self.grid_size = (100, 2**8) if grid_size is None else grid_size
        # The effective time steps used for learning and inference
        self.nt_effective = 100 if nt_effective is None else nt_effective
        self.nt = self.grid_size[0]
...
class Heat(PDE):
    ...
    def __init__(
        ...
        # Start and end time of the trajectory
        self.tmin = 0 if tmin is None else tmin
        self.tmax = 16.0 if tmax is None else tmax
